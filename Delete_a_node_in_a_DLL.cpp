...
To delete the tail of a doubly linked list, we update the linkage between its last node and its second last node.
Since a doubly linked list is bidirectional, we set the second last node's next pointer and the last node's back pointer to null. 
Then, we return the head as the result.
...
We start from the head of the doubly linked list and iterate through the list using a while loop until we reach the last node. 
The tail pointer is used to keep track of this last node.
Step 2: Obtain the second last node using the tail's back pointer, and name it the prev pointer.
Step 3: Set the 'next' pointer of the prev node to null. 
This step effectively disconnects the initial tail node from the list in the forward direction, making prev the new tail node.
